...
def finding_certain_model_classification(dataset):
    ...
    for label in sorted(TEST_DF.columns):
        if label not in get_single_value_columns(TEST_DF):
            ...
            if result:
                print(
                    f'########################## Certain Model result for {label}, time :{end_time - start_time} , test accuracy {CM_score} and RESULT ###### {result} ########################## ')
...
if __name__ == '__main__':
    ...
    for csv_filename in sorted(csv_files):
        ...
        if csv_filename.endswith('.csv') and csv_filename not in ['COVID.csv','survey.csv','uniprot_1001r_223c.csv','tsa_claims3_11.csv','classification.csv']:
