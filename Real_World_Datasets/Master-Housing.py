...
def finding_certain_model_classification(dataset):
    ...
    TEST_DF = drop_categorical_columns(data,featurize=True)
    # print(missing_values_table(TEST_DF))
    test = ['BsmtUnfSF', 'GrLivArea', 'LowQualFinSF', 'TotalBsmtSF', 'BsmtFinSF2', 'BsmtFinSF2', '2ndFlrSF', '1stFlrSF']
    test=TEST_DF.columns
    for i in sorted(test):
        df_dropped = drop_categorical_columns(data,featurize=True)
...
if __name__ == '__main__':
    ...
    path2 = os.path.join('data', 'house-prices-advanced-regression-techniques/test.csv')
    df_train = pd.read_csv(path, index_col=0)
    df_test=pd.read_csv(path2, index_col=0)
    data=pd.concat([df_train,df_test],axis=0)
    data = pd.read_csv(path, index_col=0)
    print(missing_values_table(data))
    finding_certain_model_classification(data)