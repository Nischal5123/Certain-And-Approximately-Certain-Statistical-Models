...
if __name__ == "__main__":
    df = pd.read_csv("Final-Datasets/intel-sensor-data.csv")
    label = "2"
    X, y = get_Xy(
        df, "2"
    ...
    # Drop all rows with null values in the test DataFrame
    ...
    num_label= df[label].unique()
    # Display the result
    print("Number of rows with missing values:", rows_with_missing_values)
    print("Number of Unique values in label:", df[label].nunique())
    ...
    name = f"Intel-Sensor_CM_Exist_{result}.txt"
