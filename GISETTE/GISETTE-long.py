...
def simple_imputer_test(df_train,found_seed, noise_level,trial):
...
def active_clean_test(data,noise_level,trial):
    df_dirty = make_dirty(data, found_seed_1, noise_level)
    # Access the last column by its numerical index (-1)
    df_dirty.reset_index(drop=True, inplace=True)
    ...
    filename = f'GISETTE_{noise_level*100}_{trial}.txt'
    with open(filename, 'w') as file:
...
if __name__ == '__main__':
    ...
    for noise_level in [0.001,0.01,0.1,0.05]:
        for trial in [1,2,3]:
            ...
            max_attempts=1000
            found_seed=certain_model_seed(df_train, max_attempts, noise_level,trial)
            simple_imputer_test(df_train,found_seed,noise_level,trial)
            activeclean(df_train,found_seed,noise_level,trial)
...
if found_seed_1 is not None:
    ...
    with open('GISETTE_certain_model_seed_trial1_001.txt', 'w') as file:
        ...
        file.write(
            f"time spent from certain model algorithm when certain model eixts:  {found_end_time - found_start_time}\n")
...
with open('SI_trial1_001.txt', 'w') as file:
    ...
    file.write(f"SI running time:  {simple_time}\n")
# In[ ]:
################################ Simple Imputer Finish ###############################################
