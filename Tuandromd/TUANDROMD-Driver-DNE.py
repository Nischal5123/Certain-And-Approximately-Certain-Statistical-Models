...
def certain_model_seed(df_train, max_attempts, noise_level,trial,X_test,y_test):
    ...
    if found_seed is not None:
        ...
        filename = f'DNE_TUANDROMD_{noise_level}_{trial}.txt'
...
if __name__ == '__main__':
    for noise_level in [0.001,0.01,0.1,0.05]:
        for trial in [1,2,3]:
            print("###############################       Started Missing Factor : {}..  Trial : {}  ########################################".format(
                    noise_level, trial))
            found_seed = None
            ...
            max_attempts = 10000
            found_seed = certain_model_seed(df_train, max_attempts, noise_level, trial,X_test, y_test)
            if found_seed != None:
                simple_imputer_test(df_train, found_seed, noise_level, trial, X_test, y_test)
                naive_baseline_test(df_train, found_seed, noise_level, trial, X_test, y_test)
            ...
            print(
                "###############################       Complete Missing Factor : {}..  Trial : {}    #####################################".format(
                    noise_level, trial))
